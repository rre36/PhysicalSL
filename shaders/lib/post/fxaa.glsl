...
vec3 fxaa311(vec3 color){
	...
	float lumaDown = luma(texture2DLod(colortex1,texcoord.xy+vec2(0.0f,-1.0f)*view,0.0).rgb);
	float lumaUp = luma(texture2DLod(colortex1,texcoord.xy+vec2(0.0f,1.0f)*view,0).rgb);
	float lumaLeft = luma(texture2DLod(colortex1,texcoord.xy+vec2(-1.0f,0.0f)*view,0).rgb);
	float lumaRight = luma(texture2DLod(colortex1,texcoord.xy+vec2(1.0f,0.0f)*view,0).rgb);
	...
	if (lumaRange > max(edgeThresholdMin, lumaMax * edgeThresholdMax)){
		...
		float lumaDownLeft = luma(texture2DLod(colortex1,texcoord.xy+vec2(-1.0f,-1.0f)*view,0).rgb);
		float lumaUpRight = luma(texture2DLod(colortex1,texcoord.xy+vec2(1.0f,1.0f)*view,0).rgb);
		float lumaUpLeft = luma(texture2DLod(colortex1,texcoord.xy+vec2(-1.0f,1.0f)*view,0).rgb);
		float lumaDownRight = luma(texture2DLod(colortex1,texcoord.xy+vec2(1.0f,-1.0f)*view,0).rgb);
		...
		float lumaEnd1 = luma(texture2DLod(colortex1,uv1,0).rgb);
		float lumaEnd2 = luma(texture2DLod(colortex1,uv2,0).rgb);
		...
		if (!reachedBoth){
			for(int i = 2; i < iterations; i++){
				...
				if (!reached1){
					lumaEnd1 = luma(texture2DLod(colortex1, uv1,0).rgb);
					lumaEnd1 = lumaEnd1 - lumaLocalAverage;
				}
				// If needed, read luma in opposite direction, compute delta.
				if (!reached2){
					lumaEnd2 = luma(texture2DLod(colortex1, uv2,0).rgb);
		...
		aa = texture2DLod(colortex1,finalUv,0).rgb;
	}
	return aa;
}