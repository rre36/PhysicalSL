...
float puddleH(vec3 pos, vec3 fpos){
	...
	if (mult > 0.01){
		noise = textureBicubic(noisetex,(pos.xz+frametime*0.0)/32.0).r;
	}

	noise = sin((noise + frametime) * 16.0) * 0.35 + 0.5;
	noise *= mult;
...
vec3 getPuddleNormal(vec3 pos, vec3 fpos, mat3 tbn){
    ...
    float bumpmult = 0.03;	
    pnormalmap = pnormalmap * vec3(bumpmult, bumpmult, bumpmult) + vec3(0.0f, 0.0f, 1.0f - bumpmult);
